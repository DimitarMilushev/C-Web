...
namespace SIS.Framework.Routers
{
    public class ControllerRouter : IMvcRouter
    {
        ...
        {
            var listItemPattern = $@"{paramName}_\d";
            StringBuilder result = new StringBuilder();

            foreach (var formDataKey in httpRequest.FormData.Keys)
            {
                if (Regex.IsMatch(formDataKey, listItemPattern))
                    result.Append($"{httpRequest.FormData[formDataKey]} ");
            }
